...
class ReplyEvent {
  ...
  public function wechatReply($rs){
    ...
    switch ($this->rs['segment']){
      //模型为自定义时处理流程
      case 'Activity':
		  $segment = 'Tchat_activity';
        return $reply=$this->reply($segment);
        break;
   ...
   */
  private function reply($segment="Document"){
    $rs = $this->rs;
    switch ($rs['reply_type']){
      ...
      case 'document' :
        $news= M($segment)->where(array('status'=>array('eq',1)))->field('id,title,description,cover_id,link_id')->order('id desc')->select($rs['reply_id']);
