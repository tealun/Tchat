 ...
 */
class PatternLogic {
	/* 对验证方式进行位赋值 0为验证条件组合连接符验证 1为姓名验证 2为电话号码验证 4为QQ号码验证 8为邮箱验证 */
	private $pregs = array(
		0 => array(
			'preg' => '[ \-\+]?',
			'name' => '间隔符号'
	...
	public function renewPatternArr() {
		...
		foreach ($segs as $seg) {
			...
			$map = array('startup' => array('lt', time()), 'deadline' => array('not between', array(1, time())), 'status' => array('eq', '1'), 'check_info' => array('gt', '0'));
		...
		unset($i);
		S(patternArr, $patternArr, 3600);//存进缓存，实效1小时
	}
	 ...
	 */
	public function startPreg($openId, $keyword, $pattern) {
		/*检查是否是取消指令*/
	...
	private function checkInfo($openId, $pattern, $matches) {
		...
		if ($tips) {
			...
			if (!empty($pattern['checked_reply'])) {
				...
				$reply = get_text_arr('非常感谢您的参与，所需信息已经完整，必要时我们会联系您，请留意您所留联系方式。');
			}
